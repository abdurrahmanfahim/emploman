...
const AdminLeave = () => {
  ...
  const handleConfirm = async (action, id) => {
    try {
      await api.patch(`/leave/${id}`, { status: action === 'approve' ? 'APPROVED' : 'REJECTED' })
      fetchLeaves()
    } catch (err) {
      console.error(err)
    }
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[['Total Requests', total], ['Pending', pending], ['Approved', approved]].map(([title, value]) => (
      ...
      <LeaveActionModal open={modal.open} onOpenChange={closeModal} action={modal.action} leave={modal.leave} onConfirm={handleConfirm} />
    </div>
  )
}
